...
# Relaxamento
relaxamento = 0.5
print("\nFator de relaxamento =", relaxamento)
...
while (np.max(Epest) > Eppara and k < maxit):
    ...
    for i in range(0, n):
        ...
        x_calculado = 1/A[i, i]*(b[i] - soma1 - soma2)

        x_new[i] = relaxamento*x_calculado + (1 - relaxamento)*x_old[i]
...
plt.title("Convergência do Método de Gauss-Seidel com Relaxamento")
plt.grid()
plt.show()